...
import buildRedirect from "./build_redirect";
...
const buildShortUrl = ({ req, doc }) =>
  `https://${req.hostname}/${encode(doc._id)}`;
...
export default function Server(connector, options = {}) {
  ...
  const HullMiddleware = connector.clientMiddleware({
    hostSecret,
    fetchShip: true,
    cacheShip: false
  });
  ...
  app.get("/:encoded_id", (req, res) => {
    try {
      ...
      Hull.logger.info("incoming.user.start", {
        ...logMessage,
        message: "Follow link"
      });
      // check if url already exists in database
        ...
        if (doc) {
          ...
          try {
            ...
            if (ship && long_url && organization) {
              Hull.logger.info("incoming.user.success", {
                ...logMessage,
                message: "Link Followed"
              });
              ...
              // Invalid link
              Hull.logger.error("incoming.user.error", {
                ...logMessage,
                doc,
